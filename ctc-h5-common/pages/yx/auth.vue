...
<script>
	...
	import {getAuthInfo,getUserInfo} from '@/api/app/qrcode/index.js'
	export default{
		data(){
			return{
				appid:'',//用户公众号appid,
		...
		onLoad({dealer_id,qrcode,code,batch,appid}) {
			...
			if(!code){
				...
				getAuthInfo({dealer_id:dealer_id}).then(res=>{
					//跳转授权页面
					const redirect_uri='http://yx.uni-2id.com?qrcode='+qrcode+'&batch='+batch+'&dealer_id='+dealer_id
				...
				getUserInfo({code:code,appid:appid,dealer_id:dealer_id}).then(res=>{
					...
					// 保存tokne和userId到缓存
					const url='http://yx.uni-2id.com?qrcode='+qrcode+'&batch='+batch+'&dealer_id='+dealer_id+'&code='+code+'&state=STATE'+'&appid='+appid
