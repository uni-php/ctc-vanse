	...
	<view class="container">
		...
		<view class="gift-box">
			<view class="gift-item" v-for="(item,index) in car_gifts" :key="index">
				...
				<u--image radius="8" :showLoading="true" :src="item.goods_info.cover[0].url" width="85px" height="85px"></u--image>
				<view class="recom-info">
					<u--text :text="item.goods_info.product_name" size="14"></u--text>
					...
					<view class="recom-points">
						...
						<u--text :text="'￥'+item.goods_info.sale_price" color="#ff9900" size="10"></u--text>
					</view>
					<view class="count-box">
						<lxc-count 
							@handleCount="handleCountClick(item)" 
							:value.sync="item.goods_num"
							:min=1
							:delayed="100"
							>
						</lxc-count>
			...
			<view class="cart-info">
				<view class="cart-info-sum">
					...
					<view class="sum-money">
						...
						<u--text  :text="'￥'+sum_money+'元'" color="#ff9900" size="10"></u--text>
					</view>
				</view>
	...
	export default{
		...
		computed:{
			...
			sum_money(){
				...
				const valid_gifts=this.car_gifts.filter(item=>item.is_selected.length!=0)
				valid_gifts.forEach((item,index)=>{
					sum += item.goods_info.sale_price*1 * item.goods_num
				})
				return sum
			}
		},
		methods:{
			...
			handleCountClick(item){
				const params={
					app_goods_id:item.app_goods_id,// 礼品id
