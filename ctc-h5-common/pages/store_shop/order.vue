			...
			<view class="order-item" v-for="(item,index) in orderInfo" :key="index">
				<view class="order-list" v-if="item.pay_mode=='direct'">
					...
					<view class="goods-item" @click="checkOrderInfo(item)">
						<u--image :showLoading="true" :src="item.order_goods_info.cover.length>0?item.order_goods_info.cover[0].url:''" width="90px" height="90px"></u--image>
						<view class="recom-info">
							<u--text :lines="2" :text="item.order_goods_info.product_name" size="13"></u--text>
							...
							<view class="recom-points">
								...
								<u--text :text="'￥'+item.order_goods_info.sale_price" color="#ff9900" size="12"></u--text>
							</view>
				...
				<view class="order-list" v-else>
					...
					<view class="goods-item" v-for="(it,idx) in item.order_goods_info" @click="checkOrderInfo(item)">
					...
					<view class="sum-box">
						共{{sum(item.order_goods_info)}}件商品 总金额{{item.total_price}}
					</view>
					<view class="set-box">
...
<script>
	...
	export default{
		...
		onLoad({status,store_id,dealer_id}) {
		...
		methods:{
			...
			checkOrderInfo(item){
				uni.$u.route({
					url: 'pages/store_shop/order_info',
					params: {
