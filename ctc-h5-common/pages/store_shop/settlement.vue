	...
	<view class="container">
		...
		<view class="goods-box" v-if="goods_id">
			<view class="goods-item">
				...
				<view class="recom-info">
					<u--text :lines="2" :text="goods_info.product_name" size="13"></u--text>
		...
		<view class="goods-box" v-else>
			<view class="goods-item" v-for="(item,index) in order_info">
				...
				<view class="recom-info">
					<u--text :lines="2" :text="item.product_name" size="13"></u--text>
					<u--text text="" color="gray" size="12"></u--text>
					<view class="recom-points">
						<!-- <u-icon name="integral" color="#ff9900" size="18"></u-icon> -->
						<!-- <u--text style="flex:none" :text="item.points_num" color="#ff9900" size="12"></u--text> -->
						<u--text :text="'￥'+item.sale_price" color="#ff9900" size="12"></u--text>
					</view>
					<view class="shopping-cart">
						<u--text :text="`x${item.goods_num}`" size="13" color="gray"></u--text>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="price-box">
			...
			<view class="sum-price">
				<text>应付：</text><text style="color: #fa2209;">￥{{orderTotalPrice*1 + freight*1}} </text>
				<!-- <view style="display: flex;flex-wrap: nowrap;"><u-icon name="integral" color="#ff9900" size="18"></u-icon><text style="color: #fa2209;">{{orderTotalPoints}}</text></view> -->
			</view>
		</view>
...
<script>
	...
	export default{
		...
		onLoad({cartIds,goods_id,store_id,app_goods_id,dealer_id}) {
		...
		computed:{
			orderTotalPrice(){
				if(this.goods_id){
					...
					return this.goods_info.sale_price
				}else{
					let sum=0
					this.order_info.forEach((item,index)=>{
						// if(item.type==2){
							sum += item.sale_price*1 * item.goods_num
						// }
					})
					return sum
				}
		...
		methods:{
			...
			submit(){
				...
				const params={
					...
					order_goods_info:this.goods_id?this.goods_info:this.order_info,
