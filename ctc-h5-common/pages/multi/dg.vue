		...
		<u-overlay :show="reg_show" @click="reg_show = false">
			<view class="warp">
				<view class="rule-box">
					...
					<view class="reg-btn" @click="handleRegBtn">{{reg_status==1?'查看':'我知道了'}}</view>
				</view>
			</view>	
		</u-overlay>
		<!-- 信息收集 -->
...
<script>
	...
	import {getJsConfig,getTpl,getPrize} from '@/api/app/multi/index.js'
		...
		methods:{
			...
			getPrize(){
				...
				getPrize(data).then(res=>{
					...
					//导购登记
					if(res.status==203){
						this.reg_status=res.data.status
						this.reg_show=true
					}
					if(res.status==200){
						//中奖
