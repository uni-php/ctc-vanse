    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="角色" prop="roleIds">
              <el-select v-model="form.roleIds" collapse-tags multiple placeholder="请选择">
                <el-option
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item v-if="!form.uid" label="用户密码" prop="password">
              <el-input :disabled="form.uid != undefined" v-model="form.password" placeholder="请输入用户密码" type="password" maxlength="20" show-password/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$confirm('确认要"' + text + '""' + row.username + '"用户吗?', "警告", {
          ...
          return changeUserStatus({uid:row.uid, status:row.status});
        }).then(() => {
    ...
    handleUpdate(row) {
      ...
      getUser({uid:userId}).then(response => {
        this.form = response.data;
        this.roleOptions = response.roles;
        this.form.roleIds = response.data.roleIds.split(",").map(Number)
        this.open = true;
        this.title = "修改用户";
        // this.form.password = "";
      });
    ...
    handleResetPwd(row) {
      this.$prompt('请输入"' + row.username + '"的新密码', "提示", {
        ...
        inputErrorMessage: "用户密码长度必须介于 5 和 20 之间",
          ...
          resetUserPwd({uid:row.uid, password:value}).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const userIds = row.uid || this.ids;
