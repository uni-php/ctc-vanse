...
<script>
import {getStoreGoodsClass,saveStoreGoodsClass,deleStoreGoodsClass,queryStoreGoodsClass} from '@/api/app/store/index'
export default {
    data(){
        return{
            tableData:[],
    ...
    methods:{
        getList(){
          getStoreGoodsClass(this.queryParams).then(res=>{
                this.tableData=res.data
                this.total=res.total
            })
        },
        handleEdit(row){
          queryStoreGoodsClass({id:row.id}).then(res=>{
                this.title='修改分类'
                this.dialog=true
                this.form=res.data
            })
        ...
        handleDele(row){
            this.$confirm(`确认删除${row.class_name}分类?`, '提示', {
                ...
                deleStoreGoodsClass(row).then(res=>{
                    if(res.code==200){
        ...
        submitForm(){
            this.$refs["form"].validate(valid => {
                if(valid){
                  saveStoreGoodsClass(this.form).then(res=>{
                        if(res.code==200){
                            this.$message.success(res.msg)
                            this.dialog=false
                            this.getList()
