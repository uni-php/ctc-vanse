...
<script>
import {getClassList,savePointsClass,delePointsClass,queryPointsClass} from '@/api/app/points'
export default {
    data(){
        return{
            tableData:[],
    ...
    methods:{
        ...
        handleEdit(row){
            queryPointsClass({id:row.id}).then(res=>{
                this.title='修改分类'
                this.dialog=true
                this.form=res.data
            })
        ...
        submitForm(){
            this.$refs["form"].validate(valid => {
                if(valid){
                    savePointsClass(this.form).then(res=>{
                        if(res.code==200){
                            this.$message.success(res.msg)
                            this.dialog=false
                            this.getList()
