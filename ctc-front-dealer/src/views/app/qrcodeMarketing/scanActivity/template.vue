...
<script>
import {getTplList} from '@/api/app/qrcode'
export default {
    data(){
        return{
            tplDialog:false,
    ...
    methods:{
        ...
        editTpl(item){
            this.$router.push({path:'/qrcodeMarketing/sacnaAtivity/addTpl',query:{type:1,id:item.id}})
        },
        createTpl(val){
            this.$router.push({path:'/qrcodeMarketing/sacnaAtivity/addTpl',query:{type:val}})
        },
        query(){
