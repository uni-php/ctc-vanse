<template>
  <div class="app-container">
    ...
    <el-table
        ...
        <el-table-column prop="num" align="center" label="门店数量"></el-table-column>
...
<script>
import {getStoreTypeList,saveStoreType,queryStoreTypeInfo,deleStoreType} from '@/api/customer/store'
export default {
    data(){
        return{
            title:'',
    ...
    methods:{
        getList(){
            getStoreTypeList(this.queryForm).then(res=>{
        ...
        submitForm(){
            this.$refs["form"].validate(valid => {
                if(valid){
                    saveStoreType(this.form).then(res=>{
                       if(res.code==200){
        ...
        handleEdit(row){
            queryStoreTypeInfo({id:row.id}).then(res=>{
                this.form=res.data
                this.dialogVisible=true
                this.title='修改类型'
            })
        ...
        handleDele(row){
            this.$confirm(`确定删除${row.class_name}类型`, '提示', {
                ...
                if(row.num!=0){
                    this.$message.warning('该类型下存在门店，不可删除')
                    return;
                }
                deleStoreType({id:row.id}).then(res=>{
                    if(res.code==200){
                        this.$message.success(res.msg)
                        this.getList()
                    }else{
