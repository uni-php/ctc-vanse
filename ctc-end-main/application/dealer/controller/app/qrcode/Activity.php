...
class Activity extends Controller
{
    ...
    public function saveActivity(Request $request){
        ...
        if(array_key_exists('id',$data)){
            $res=Db::name('dealer_qrcode_marketing_activity')->strict(false)->update($data);
        }else{
            $res=Db::name('dealer_qrcode_marketing_activity')->strict(false)->insert($data);
        }
        if($res||$res==0){
    ...
    public function getActivityList(Request $request){
        $params=$request->get();
        $list=Db::name('dealer_qrcode_marketing_activity')
            ...
            ->select();
        $total=Db::name('dealer_qrcode_marketing_activity')
            ->where('dealer_id',$this->dealer_id)
            ->where('code_batch_data','like','%'.$params['code_batch'].'%')
            ->where('activity_name|id|remark','like','%'.$params['keywords'].'%')
            ->where('status','<>',3)
    ...
    public function setActivityStatus(Request $request){
        $params=$request->get();
        $res=Db::name('dealer_qrcode_marketing_activity')
            ->where('dealer_id',$this->dealer_id)
            ->where('id',$params['id'])
            ->update([
    ...
    public function getActivityInfo(Request $request){
        ...
        $res['work_type']=json_decode($res['work_type'],true);
