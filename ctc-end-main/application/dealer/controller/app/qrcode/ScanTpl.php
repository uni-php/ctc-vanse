...
class ScanTpl extends Controller
{
    ...
    public function saveScanTpl(Request $request){
        ...
        if(array_key_exists('id',$data)){
            $res=Db::name('dealer_qrcode_marketing_tpl')->update($data);
        }else{
            $res=Db::name('dealer_qrcode_marketing_tpl')->insert($data);
        }
        if($res||$res==0){
    ...
    public function getTplList(Request $request){
        ...
        $list=Db::name('dealer_qrcode_marketing_tpl')
        ...
        $total=Db::name('dealer_qrcode_marketing_tpl')
            ->where(new Where($condition))
            ->where('dealer_id',$this->dealer_id)
            ->count();
        $res=[
    ...
    public function getTplInfo(Request $request){
        $id=$request->param('id');
        $res=Db::name('dealer_qrcode_marketing_tpl')
            ->where('id',$id)
            ->find();
