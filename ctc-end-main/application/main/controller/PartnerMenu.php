...
class PartnerMenu extends Controller
{
    //查询菜单数据
    public function getMenu(Request $request){
        $params=$request->get();
        $list=Db::name('partner_menu')
...
//                ->where('menuId',$menuId)
    ...
    public function delMenu(Request $request){
        $id=$request->param('menuId');
        $res=Db::name('partner_menu')->delete($id);
        if($res){
            $info=[
                'code'=>200,
                'msg'=>'删除成功'
