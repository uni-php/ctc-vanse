...
class Cart extends Controller
{
    public function addCart(Request $request){
        $params=$request->get();
        // 相同规格 相同商品id  存在时累计加1
        $has=Db::name('dealer_store_shop_cart')
            ->where('goods_sku_id',$params['goods_sku_id'])
            ->where('dealer_id',$params['dealer_id'])
            ->where('app_goods_id',$params['app_goods_id'])
            ->where('store_id',$params['store_id'])
            ->where('openid',$params['openid'])
            ->where('is_delete',0)
            ->find();
        if($has){
            $res=Db::name('dealer_store_shop_cart')->where('id',$has['id'])->setInc('goods_num');
        }else{
            $res=Db::name('dealer_store_shop_cart')->insert($params);
        }
        if($res){
    ...
    public function getCartList(Request $request){
        $params=$request->get();
        $list=Db::name('dealer_store_shop_cart')
            ->where('dealer_id',$params['dealer_id'])
            ->where('store_id',$params['store_id'])
            ->where('openid',$params['openid'])
            ->where('is_delete',0)
        ...
        foreach ($list as $key=>$value){
            $gift_info=Db::name('dealer_store_app_goods')
                ->alias('a')
                ->join('dealer_product b','a.product_id=b.id')
                ->where('a.id',$value['app_goods_id'])
        ...
        $sum=Db::name('dealer_store_shop_cart')
            ->where('dealer_id',$params['dealer_id'])
            ->where('openid',$params['openid'])
            ->where('store_id',$params['store_id'])
            ->where('is_delete',0)
    ...
    public function updateCart(Request $request){
        $params=$request->get();
        $res=Db::name('dealer_store_shop_cart')
            ->where('goods_sku_id',$params['goods_sku_id'])
            ->where('dealer_id',$params['dealer_id'])
            ->where('app_goods_id',$params['app_goods_id'])
            ->where('openid',$params['openid'])
    ...
    public function clearCart(Request $request){
        ...
        try{
            foreach ($params['cart_ids'] as $key=>$value){
                Db::name('dealer_store_shop_cart')
                    ->where('dealer_id',$params['dealer_id'])
                    ->where('openid',$params['openid'])
                    ->where('store_id',$params['store_id'])
                    ->where('id',$value)
