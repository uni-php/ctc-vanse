...
//                Db::name('dealer_member')->where('openid',$data['openid'])->setDec('points', number_format($data['total_points'],2));
                ...
                if($data['pay_mode']=='cart'){
                    ...
                    foreach ($data['order_goods_info'] as $key =>$value){
                        Db::name('dealer_store_app_goods')
                            ->where('id',$value['app_goods_id'])
                    ...
                    Db::name('dealer_store_app_goods')
                        ->where('id',$data['order_goods_info']['app_goods_id'])
                        ->setInc('sales', 1);
                    Db::name('dealer_product')
                ...
                Db::name('dealer_store_shop_order')->strict(false)->insert($data);
                Db::commit();
    ...
    public function payOrder(Request $request){
        $params=$request->post();
        $orderInfo=Db::name('dealer_store_shop_order')
            ->where('openid',$params['openid'])
            ->where('dealer_id',$params['dealer_id'])
            ->where('order_id',$params['order_id'])
            ->where('store_id',$params['store_id'])
