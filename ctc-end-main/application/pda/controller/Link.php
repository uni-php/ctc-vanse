...
class Link extends Controller
{
    ...
    public function queryCodeLink(Request $request){
        ...
        if(count($secret_key)==0){
            ...
            if(strlen($params['code'])!=$batch_info['code_length']) return JsonService::returnData(500,'该物流码无效');
            ...
            if($batch_info['order_begin']<=$decrypt_code&&$decrypt_code<=$batch_info['order_end']){
                ...
                return JsonService::returnData(200,'查询成功',['code'=>$codeList,'log'=>$log,'info'=>$info]);
            }else{
                ...
                if($val['length']==strlen($params['code'])){
                    ...
                    if($order[0]<=$decrypt_code&&$decrypt_code<=$order[1]){
                        ...
                        if($batch_info['logistics_level']&&$batch_info['logistics_level']>1){
                            ...
                            //补零
                            return JsonService::returnData(200,'查询成功',['code'=>$arr,'log'=>$log,'info'=>$info]);
                        }
                    }else{
                        return JsonService::returnData(400,'该物流码无效',$decrypt_code);
                        break;
                    }
                }
            }
